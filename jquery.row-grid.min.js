...
for(b=0;b<q;++b){l+=r[b].outerWidth;e.push(f[b]);if(b===q-1)for(var c=0;c<e.length;c++)0===c&&(e[c].className+=" "+a.lastRowClass),console.log(e[c]),e[c].style.marginRight=c<e.length-1?a.minMargin+"px":0;if(l+a.maxMargin*(e.length-1)>d){var m=l+a.maxMargin*(e.length-1)-d,c=e.length;if((a.maxMargin-a.minMargin)*(c-1)<m)var k=a.minMargin,m=m-(a.maxMargin-a.minMargin)*(c-1);else k=a.maxMargin-m/(c-1),m=0;for(var s,n=0,c=0;c<e.length;c++){s=e[c];var p=r[b+parseInt(c)-e.length+1].outerWidth,g=p-p/l*m,
p=Math.round(r[b+parseInt(c)-e.length+1].height*(g/p));0.5<=n+1-g%1?(n-=g%1,g=Math.floor(g)):(n+=1-g%1,g=Math.ceil(g));s.style.cssText="width: "+g+"px;height: "+p+"px;margin-right: "+(c<e.length-1?k:0)+"px";0===c&&(s.className+=" "+a.firstItemClass)}e=[];l=0}}}h.fn.rowGrid=function(d){if(0==this.length)return console.error('No element found for "'+this.selector+'".'),this;if(1<this.length)return this.each(function(){h(this).rowGrid(d)});if("appended"===d){d=this.data("grid-options");var a=this.children("."+
d.lastRowClass),a=a.nextAll().add(a);k(this[0],d,a)}else if(d=h.extend({},h.fn.rowGrid.defaults,d),this.data("grid-options",d),k(this[0],d),d.resize)h(window).on("resize",{container:this[0]},function(a){k(a.data.container,d)});return this};h.fn.rowGrid.defaults={minMargin:null,maxMargin:null,resize:!0,lastRowClass:"last-row",firstItemClass:null}})(jQuery);